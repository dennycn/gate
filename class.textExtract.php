...
class textExtract {
    ...
    function procEncoding() {
        ...
        if( $this->isGB )
            $this->rawPageCode = iconv( 'GBK', 'UTF-8//IGNORE', $this->rawPageCode );
    }
...
//        $content = $this->convertPageCode;
        ...
        // 4.2. <div style="display:none">  " --is a problem
        $pattern = '/<div style=(.*?)display:none(.*?)>(.*?)<\/div>/is';
        $replacement = '';
        $content = preg_replace( $pattern, $replacement, $content );
//        print_r($content);
...
//		$content = preg_replace( $pattern, $replacement, $content );
    ...
    function getTextLines( $rawText ) {
        ...
        foreach( $lines as $line ) {
            ...
            $tmp = str_replace("<p><br /></p>\n", "<br />", $line);
            $this->textLines[] = $tmp;
        }
//        print_r($this->textLines);
    }
